...
func SetBytes(key string, data []uint8, expired time.Duration) error {
    ...
    if expired < time.Second {
        ...
        result, err = redis.String(con.Do(op_SETEX, prekey+key, int64(expired.Seconds()), data))
    }
...
func Delete(key string) (int, error) {
    ...
    cnt, err := redis.Int(con.Do(op_DEL, prekey+key))
    if err != nil {
        return 0, errors.New("Delete:" + err.Error())
    }
...
func LPush(key string, value interface{}) (data int, err error) {
    ...
    if err != nil {
        return ret, errors.New("LPush:" + err.Error())
    }
    return ret, nil
}
...
func SetNXExpired(key string, val interface{}, expired time.Duration) (res bool, err error) {
    ...
    enc := gob.NewEncoder(buffer)
    if err := enc.Encode(val); err != nil {
        return false, errors.New("Set:encode:" + err.Error())
    }
    ...
    result := ""
    result, err = redis.String(con.Do(op_SET, prekey+key, buffer.String(), "EX", int(expired.Seconds()), "NX"))
